enrichmentFromSRPEG <- function (universeFile, interestingGenes, subOntology){
  ...
  if (length(mysummary) == 3 & as.numeric(mysummary[3]) > 30 ){ # Test si il y a des GO significatifs + bug quand un seul GO est significatif ? d'ou cette deuxieme condition ###
    ...
    numsignif <- 30 # si pas de GO significatifs alors on print les 30 premiers termes
  }
  ...
  List2Return <- list(allRes, myGOdata, resultElim)
  #List2Return <- allRes
  return(List2Return)
